...
class TaskListener(TaskConfig):
    def __init__(self):
        super().__init__()
        # From ok30.9.25 - Enhanced media tracking
        ...
        # From ok30.9.25 - Enhanced part tracking
        self.total_parts = 1
        self.current_part = 1
        self.original_name = ""

        # New series detection features
        self.is_series = False
        self.series_pattern = None
        self.episode_files = {}

    async def on_task_created(self):
        # From ok30.9.25 - Better status handling
    ...
    async def update_and_log_status(self, new_status):
        # From ok30.9.25 - Enhanced logging
    ...
    async def start_download(self):
        # Common implementation from both branches
        if (
            not self.is_jd
            and not self.is_nzb
    ...
    async def on_download_complete(self):
        try:
            ...
            # Enhanced extraction from ok30.9.25
            ...
            # Get task-specific files only (NEW FEATURE)
            ...
            # Detect and organize series (NEW FEATURE)
            ...
            # Recalculate total size for accurate reporting (from ok30.9.25)
            if len(files_to_process) > 1:
                total_size = 0
            ...
            # Enhanced part tracking from ok30.9.25
    ...
    async def _upload_series_ordered(self):
        """Upload series files in episode order (NEW FEATURE)"""
    ...
    async def _cleanup_after_leech(self):
        """Clean up after leech task completion - Enhanced from ok30.9.25"""
    ...
    async def _process_single_video(self, up_path):
        ...
        # Reset media-specific attributes for each file (from ok30.9.25)
        self.media_info = None
        self.streams_kept = None
        self.streams_removed = None
        self.art_streams = None
        ...
        # Enhanced media processing from ok30.9.25
        ...
        # Get file modification time before upload (from ok30.9.25)
        try:
    ...
    def _format_stream_info(self, stream, stream_type):
        # From ok30.9.25 - Enhanced stream formatting
        details = []
    ...
    async def _send_leech_completion_message(self, sent_message, file_mtime):
        # From ok30.9.25 - Enhanced completion messages
    ...
    # Common methods from both branches (upload complete, error handling, etc.)
